...
export const PurchaseOrderTemplate: React.FC<POProps> = ({ data, templateConfig, currencySymbol }) => {
  ...
  const documentNumberLabel = purchaseData.documentNumberLabel || 'Purchase Order Number';
  const documentDateLabel = purchaseData.documentDateLabel || 'Date';
  const partySectionLabel = purchaseData.partySectionLabel || 'Vendor';
  ...
  return (
    <div className="bg-white p-8 font-sans text-sm text-black">
            ...
            <div className="text-right">
              ...
              <div className="mt-2 text-sm text-black">
                ...
                {isFieldVisible(resolvedConfig, 'header', 'dateLabel') && (
                  <div className="font-semibold">
                    {documentDateLabel}: {purchaseData.date || ''}
                  </div>
                )}
              </div>
            </div>
      ...
      {(supplierVisible || shipToVisible) && (
        <div className="mb-6 grid grid-cols-2 gap-6 rounded-lg bg-emerald-100 px-6 py-6 md:grid-cols-2">
          {supplierVisible && (
            <div className="space-y-1">
                ...
                itemsWithCalculations.map((item: any, index: number) => (
                  ...
                  >
                    {tableColumns.map((column) => {
                      ...
                      return (
                        <td key={column.key} className="align-top p-2 text-center">
                          {content}
                        </td>
                      );
                    })}
...
export const PurchaseOrderTemplatePreview: React.FC = () => (
  <TemplatePreviewWrapper defaultConfigFactory={createPurchaseOrderDefaultConfig} />
);
